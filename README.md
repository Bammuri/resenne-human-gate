...
**DOCKPAD(8키 AI 컨트롤러, `demo/arduino-simulator/`)의 실기기 AI-DLC 시연**을 담은 무편집 영상입니다.
실제 Claude Code를 PTY에서 구동하고 물리 키로 AI-DLC 단계·승인·거절을 전달하는 모습이며, **팀이 실기기로
확인한 사람 검증·시연 결과이고 자동 테스트가 증명하지 않습니다.** GitHub README에서는 인라인 재생 대신
...
> 위 **시연 영상**은 **DOCKPAD(8키)의 실기기 AI-DLC 시연**입니다(사람 검증·시연). 루트 2버튼 게이트
> 앱의 **① 승인 대기 → ② 승인 후 실행 → ③ 거절 후 해당 요청 미실행** PTY 키응답 경로는 이 영상과
> 별개이며, 위 **구현과 검증 범위** 표와 아래 상세 절에 정리했습니다.
