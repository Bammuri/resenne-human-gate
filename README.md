...
사람이 승인·거절로 답합니다. 지금 검증된 예시는 **Claude Code**입니다. AI가 준비하면
OLED가 사람을 부르고, 버튼을 누르면 실제로 코드가 바뀝니다. 브라우저에서 쉘과 Claude를
...
물리·웹 시뮬레이션 입력을 **Python PTY 키응답**으로 실제 Claude Code의 대기 중 요청에 전달합니다 —
...
정지시키고 `allow`/`deny`를 반환하는 승인 브로커(`server.py`의 `/api/approval*` 엔드포인트 +
`hooks/hook_bridge.py`)를 이 저장소에 **코드로 통합**하고, **단위테스트 + 서브프로세스 E2E 테스트
(`tests/test_approval.py`)와 실서버 curl 스모크**로 검증했습니다. 브라우저 승인 패널 **`/gate.html`에서
사람이 대기 중 도구 실행에 ✓ 허용 / ✕ 거부를 브로커로 제출할 수 있습니다** — 진짜 `hook_bridge.py`
프로세스 ↔ 브로커 ↔ 브라우저(ui 토큰) resolver 체인이 소프트웨어로 E2E 검증됩니다. 팀 실기기 PoC에서
버튼→브리지 왕복까지 확인됐지만(Node Bridge↔Python 어댑터 연결은 통합 남은 단계), **실행 중인 진짜 `claude`가 그 결정을 존중해
...
*완전* 소프트웨어 E2E로 검증됩니다.** 제출 저장소의 `hooks/claude_state_hook.py`는 권한을 결정하지 않는 상태 신호입니다
...
설계 범위는 공통 프로토콜과 하네스별 어댑터를 통한 AI 하네스 연동이며, **현재 검증된(코드 통합·단위·소프트웨어 E2E)
...
활성화합니다. 소프트웨어 체인(훅 프로세스 ↔ 브로커 ↔ 브라우저 resolver)은 테스트·curl 스모크로
