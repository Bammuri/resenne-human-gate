...
// D0/D1 Serial1 DFPlayer Pro DFR0768. D10-D13 and A1-A3 are unused.
#include <WiFiS3.h>
#include <EEPROM.h>
#include <Wire.h>
...
bool displayCommand(String value) {
  ...
  if (!s.length() || s.length()>14 || !numberIn(l,0,p=="workflow"?5:3)) return false;
...
bool processCommand(const String& command) {
  ...
  String at;
  if (command=="audio:status") at="AT+QUERY=5";
...
void scanInputs() {
  ...
  for (uint8_t i=0;i<8;i++) {
    ...
    if (now-buttons[i].changed>=30 && reading!=buttons[i].stable) {
      ...
      if (reading==LOW) emitLine("BUTTON_LAB_KEY:"+String(i+1));
    }
  }
  if (now-potAt>=10) {
    potAt=now;
    potFiltered=(potFiltered*3+analogRead(A0))/4;
...
void renderDisplay() {
  ...
  else if (profile=="workflow") { const char* modes[]={"MANUAL","GUIDE","STEP","GATED AUTO","AUTO BUILD","FULL AUTO"}; detail=modes[level]; }
...
void handleHttp() {
  ...
  String key;int k=lower.indexOf("x-board-token:");
  if(k>=0){int end=headers.indexOf("\r\n",k);key=headers.substring(k+14,end<0?headers.length():end);key.trim();}
  if(method=="POST" && path=="/wifi") key=formValue(body,"key");
  if(key!=BOARD_API_TOKEN){respondHttp(401,"{\"error\":\"Invalid board key\"}");return;}
...
void startNetwork() {
  ...
  if(apMode) status=WiFi.beginAP(BOARD_AP_SSID,BOARD_AP_PASSWORD);
...
void setup() {
  ...
  potFiltered=potLastSent=analogRead(A0); // No pull-up, no startup action.
  Wire.begin();
  for(uint8_t address=0x3C;address<=0x3D && !oledReady;address++){Wire.beginTransmission(address);if(Wire.endTransmission()==0)oledReady=display.begin(SSD1306_SWITCHCAPVCC,address);}
...
void loop(){scanInputs();readSerial();serviceNetwork();renderDisplay();}
