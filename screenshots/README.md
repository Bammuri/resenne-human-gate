...
**실물 하드웨어 사진**, 그리고 **팀 실기기 무편집 시연 영상(`demo.mp4`)**이 들어 있습니다.
...
- **`demo.mp4`** — 팀이 실기기로 촬영한 **무편집 시연 영상**(사람 검증·시연, 자동 테스트가 증명하지 않음).
  약 8.9MB. GitHub에서는 인라인 재생 대신 파일 링크로 열립니다.

## 루트 2버튼 게이트 — PTY 키응답 시연 근거

위 `demo.mp4`는 팀이 실기기로 확인한 무편집 시연 근거입니다. 루트 HUMAN GATE 게이트 앱의 PTY 키응답
경로는 아래 ①~③ 순서로 시연하며, 저장소 코드·테스트와 루트 `README.md`의 **구현과 검증 범위** 표로 뒷받침합니다.
